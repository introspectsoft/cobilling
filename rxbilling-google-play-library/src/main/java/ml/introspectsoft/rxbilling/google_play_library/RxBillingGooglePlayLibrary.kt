...
class RxBillingGooglePlayLibrary(
    ...
    @CheckReturnValue
    override fun queryInAppPurchases(vararg skuIds: String): Observable<Inventory?> = query(
            BillingClient.SkuType.INAPP, skuIds.toList(), SkuDetails::toInventory
    )

    @CheckReturnValue
    override fun querySubscriptions(vararg skuIds: String): Observable<Inventory?> = query(
    ...
    @get:CheckReturnValue override val isBillingForInAppSupported
        get() = Completable.complete()
                .subscribeOn(scheduler) // https://issuetracker.google.com/issues/123447114

    @get:CheckReturnValue override val isBillingForSubscriptionsSupported
        get() = Completable.complete()
                .subscribeOn(scheduler) // https://issuetracker.google.com/issues/123447114

    @CheckReturnValue
    override fun purchase(
            inventory: Inventory, developerPayload: String
    ...
    @get:CheckReturnValue override val purchasedInApps: Observable<Purchased>
    ...
    @get:CheckReturnValue override val purchasedSubscriptions: Observable<Purchased>
    ...
    @CheckReturnValue
    fun <T> getPurchased(skuType: String, converter: (Purchase) -> T) =
            connect().flatMapObservable { client ->
                Observable.create<T> { emitter ->
                    // TODO: Check if this works instead of purchaseHistoryAsync
                    var result = client.queryPurchases(skuType)
                    if (result.responseCode == RxBilling.BillingResponse.OK) {
